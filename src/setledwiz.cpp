...
#define J_PRODUCT_ID 0x00f0
#define J_REPORT 0x0200
#define J_MESG_LENGTH 8
...
auto apply49mode(long int modeId) {
   ...
   if (device == nullptr) {
     ...
     /*
        ...
        LWZ_SBA (active1,active2,active3,active4,pulse_speed): 

 	lz_array = [active1, active2, active3, active4,pulse_speed]
	msg = [64]+lz_array+[0,0]
	dev.ctrl_transfer(0x21, 0x09, 0x0200, 0, msg)
    
    LWZ_PBA (color1,color2,color3,color4,color5,color6,color7,color8):
    #send message to device
	msg = [color1,color2,color3,color4,color5,color6,color7,color8]
	dev.ctrl_transfer(0x21, 0x09, 0x0200, 0, msg)
    
    // activates all 4 banks of 8 outputs
LWZ_SBA(255,255,255,255,7)
 
//sends to each bank of 8 outputs
LWZ_PBA(48,0,0,48,0,0,48,0)
LWZ_PBA(0,48,0,0,48,0,0,48)
LWZ_PBA(0,0,48,0,0,48,0,0)
LWZ_PBA(48,0,0,48,0,0,0,0)


time.sleep(2)
LWZ_SBA(0,0,0,0,0)
time.sleep(2)
https://github.com/MikeCurrington/LedPinky/blob/master/LedWiz.py

    */

        unsigned char message[J_MESG_LENGTH] = { 64, 255, 255, 255, 255, 7, 0, 0 };   // 49-way
        ...
        rc = libusb_control_transfer(devicehandle,
         ...
         unsigned char message2[J_MESG_LENGTH] = { 48, 48, 48, 48, 48, 48, 48, 48 };   // 49-way
